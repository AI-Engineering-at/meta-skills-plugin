...
def validate(path: Path, strict: bool = False) -> dict:
    ...
    if not meta.get("name"):
        ...
        warnings.append(f"name '{meta['name']}' should be lowercase-kebab (active verb: deploy-service, not ServiceDeployment)")
    ...
    if not meta.get("description"):
        ...
        if desc_lines > 5:
            warnings.append(f"Description is {desc_lines} lines (recommend <=3 for token efficiency)")
    ...
    if abs_paths:
        issues.append(f"Hardcoded absolute paths found: {abs_paths[:3]}. Use relative paths or ${{CLAUDE_PLUGIN_ROOT}}")
    ...
    claude_only = re.findall(r"\b(EnterPlanMode|ExitPlanMode|NotebookEdit|TodoWrite)\b", body)
    if claude_only:
        warnings.append(f"Claude-Code-specific tools used: {claude_only}. May not work in Cursor/ChatGPT")
    ...
    if strict:
        ...
        if tools_str.startswith("["):
            ...
            if len(tools) > 6:
                warnings.append(f"[strict] {len(tools)} tools — each costs ~200 tokens context. Reduce if possible.")
